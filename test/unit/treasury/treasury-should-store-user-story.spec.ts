...
const treasuryShouldStoreUserStory = async (): Promise<void> => {
    ...
    describe("Testing User Story Treasury", async () => {
        beforeEach(async () => {
            await deployments.fixture(["deploy-time-lock", "deploy-user-story-treasury"]);
            const TimeLock = await deployments.get("TimeLock");
            timeLock = (await ethers.getContractAt("TimeLock", TimeLock.address)) as TimeLock;
        ...
        context("Testing Function storeUserStory()", async () => {
            it("should only allow Owner to store a new User Story", async () => {
                ...
                await expect(userStoryTreasury.storeUserStory(description, functionalComplexity, effortEstimation)).to.be.revertedWith("Ownable: caller is not the owner")
            });
            ...
            it.skip("should store a new User Story", async () => {
                ...
                await userStoryTreasury.connect(timeLockSigner).storeUserStory(description, functionalComplexity, effortEstimation);
